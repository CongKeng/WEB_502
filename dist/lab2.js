...
listProducts.forEach((p) => {
    console.log(`Tên: ${p.name}, Giá: ${p.price}, Sale: ${p.sale}, Đánh giá: ${p.rate}`);
});
// Viết hàm tính tổng giá bán sản phẩm (sử dụng reduce)
const totalPrice = listProducts.reduce((sum, products) => sum + products.price, 0);
console.log("Tổng giá:", totalPrice);
// Viết hàm lọc những sản phẩm đang Sale và đánh giá từ Trung bình trở lên (Sử dụng filter)
const saleProducts = listProducts.filter((p) => p.sale && (p.rate === Rate.medium || p.rate === Rate.hight));
